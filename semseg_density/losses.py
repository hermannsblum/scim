  ...
  def _aux_forward(self, *inputs, **kwargs):
    *preds, target = tuple(inputs)
    ...
    for i in range(1, len(preds)):
      aux_loss = super(MixSoftmaxCrossEntropyLoss,
  ...
  def __init__(self,
    ...
    if use_weight:
      ...
      weight = torch.FloatTensor([
          ...
          1.0023, 0.9539, 0.9843, 1.1116, 0.9037, 1.0865, 1.0955, 1.0865,
          1.1529, 1.0507
      ])
...
class MixSoftmaxCrossEntropyOHEMLoss(SoftmaxCrossEntropyOHEMLoss):
  ...
  def __init__(self, aux=False, aux_weight=0.2, ignore_index=-1, **kwargs):
    super(MixSoftmaxCrossEntropyOHEMLoss,
          self).__init__(ignore_label=ignore_index, **kwargs)
    self.aux = aux
    self.aux_weight = aux_weight
