...
from deeplab.oaisys_sampling import get_deeplab_embeddings, get_deeplab, get_sampling_idx, get_dino_embeddings, dino_color_normalize, get_dino
...
def get_distances(subset, shard, subsample, device, pretrained_model,
  ...
  if normalize:
    out['dino_features'] = sklearn.preprocessing.normalize(out['dino_features'],
                                                           norm='l2')
  out['distances'] = sp.spatial.distance.pdist(out['dino_features'],
                                                    metric='cosine')
...
@ex.capture
def clustering_based_inference(features, clustering, subset, pretrained_model, expected_feature_shape,
                               feature_name, device, normalize, use_euler, num_classes, _run):
  # set up NN index to find closest clustered sample
  knn = hnswlib.Index(space='l2', dim=768)
  ...
  deeplab_model, hooks = get_deeplab(pretrained_model=pretrained_model,
                             feature_name=feature_name,
                             device=device,
                             num_classes=num_classes)
  ...
  for idx, blob in tqdm(enumerate(data)):
    ...
    # run inference
    out = dino_model.get_intermediate_layers(image, n=1)[0]
    h = int(image.shape[2] / dino_model.patch_embed.patch_size)
    w = int(image.shape[3] / dino_model.patch_embed.patch_size)
    ...
    cluster_pred = cluster_pred.reshape((h, w))
    cluster_pred = cv2.resize(cluster_pred, (640, 480),
                              interpolation=cv2.INTER_NEAREST)
    # save output
    np.save(os.path.join(directory, f'{idx:06d}_dino{_run._id}.npy'),
            cluster_pred)
...
def get_hdbscan(apply_scaling, same_voxel_close, distance_activation_factor,
                cluster_selection_method, min_cluster_size, min_samples, subset,
                shard, subsample, device, pretrained_model, feature_name,
                pred_name, uncert_name, uncertainty_threshold,
                expected_feature_shape, normalize, use_euler, _run):
  out = distance_preprocessing(subset=subset,
                      shard=shard,
                      subsample=subsample,
                      device=device,
                      pretrained_model=pretrained_model,
...
def best_hdbscan(
  ...
  clustering_based_inference(features=out['dino_features'],
