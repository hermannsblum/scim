...
from deeplab.marsscapes_sampling import get_deeplab_embeddings, get_deeplab, get_sampling_idx
...
def clustering_based_inference(features, clustering, subset, pretrained_model,
  ...
  for image_name in tqdm(os.listdir(test_path)):
    ...
    cluster_pred = cv2.resize(cluster_pred, (512, 512),
                              interpolation=cv2.INTER_NEAREST)
    # save output
    np.save(os.path.join(directory, f'{image_name[:-4]}_seg{_run._id}.npy'),
            cluster_pred)
...
ex.add_config(
    ...
    expected_feature_shape=[64, 64],
    pred_name='ensemble_pred',
    uncert_name='ensemble_maxlogit-pp',
    ignore_other=True,
    apply_scaling=True,
    same_voxel_close=None,
    uncertainty_threshold=-4.7,
