...
def save_checkpoint(model, postfix=None):
  """Save Checkpoint"""
  filename = 'deeplab_oaisys_adapted_gt.pth'
  ...
  if postfix is not None:
    best_filename = f'deeplab_oaisys_adapted_gt_{postfix}.pth'
...
def train(
  ...
  # filter labels > 15 for those that actually exist
  label_counts = np.zeros(1, dtype=np.int64)
  for idx in range(len(data)):
    label = np.load(os.path.join(directory,
                                   f'{idx}_{pseudolabels}.npy')).squeeze()
    count = np.bincount(label[label >= 15])
  ...
  new_cluster = 15
  for i in range(15, label_counts.shape[0]):
  ...
  # upload checkpoint
  _run.add_artifact(os.path.join(TMPDIR, 'deeplab_oaisys_adapted_gt_best.pth'))
  time.sleep(5)
  if use_euler:
